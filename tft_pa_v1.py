# ==========================================================
# TFT v1 — otimizado para GPU 6GB
# Previsão de Jan/2025 até Dez/2034 (120 meses)
# Inclui:
#   ✅ AGREGAÇÃO MENSAL DOS DADOS
#   ✅ NÍVEL 3: PREVISÃO COM QUANTIS
#   ✅ Integração de projeções REAIS do IBGE por sexo/idade
#   ✅ Integração de projeções REAIS de IPCA e PIB
#   ✅ Integração com dados epidemiologicos
#   ✅ Seção de Avaliação de Métricas e Comparação com Baseline (Regressão Linear)
# ==========================================================
...
if __name__ == "__main__":
    ...
    # ... (Caminhos e configs iniciais) ...
    BASE_PATH = "C:/projects/tcc/"
    CSV_PATH = BASE_PATH + "dados_top10_consolidadosPA_2015_2024.csv"
    ...
    OUT_PATH = BASE_PATH + "predicoes_TFT_v1.csv"
    CHECKPOINT_PATH = BASE_PATH + "tft_v1.ckpt"
    ...
    df_raw = pd.read_csv(CSV_PATH, low_memory=False, usecols=["PA_PROC_ID", "Ano", "Mes", "Valor_Total", "Quantidade"])
    # ... (código de agregação de gastos) ...
    df_raw["PA_PROC_ID"] = df_raw["PA_PROC_ID"].astype(str)
    df_raw["Valor_Total"] = pd.to_numeric(df_raw["Valor_Total"], errors="coerce").astype("float32")
    df_raw = df_raw.dropna(subset=["Valor_Total", "Ano", "Mes", "PA_PROC_ID"])
    df = df_raw.groupby(["PA_PROC_ID", "Ano", "Mes"]).agg(
    ...
    df = df.sort_values(["PA_PROC_ID", "Ano", "Mes"]).reset_index(drop=True)
    df["time_idx"] = df.groupby("PA_PROC_ID").cumcount().astype("int32")

    print("✅ Dados enriquecidos com projeções e dados epidemiológicos.")
    ...
    training = TimeSeriesDataSet(
        ...
        time_idx="time_idx", target="Valor_Total", group_ids=["PA_PROC_ID"],
        ...
        static_categoricals=["PA_PROC_ID"], allow_missing_timesteps=True
    )
    ...
    # ----------------------------------------------------------
    # 7) Criar ou carregar o modelo com QUANTIS
    # ----------------------------------------------------------
    # O código é idêntico ao da versão anterior, pois a lógica de treino e inferência
    # já está preparada para lidar com as novas colunas que adicionamos.
    # A única diferença será o comportamento do modelo, que agora aprenderá
    # com as projeções reais do IBGE.
    ...
    if model_exists:
        ...
        print("⚙️ Criando modelo TFT para prever quantis com features do IBGE…")
    ...
    # ----------------------------------------------------------
    # 8) AVALIAÇÃO E BENCHMARKING
    # ----------------------------------------------------------
    print("\n📊 Iniciando Avaliação e Benchmarking no Conjunto de Validação...")

    df_train = df[lambda x: x.time_idx <= training_cutoff].copy()
    df_val = df[lambda x: x.time_idx > training_cutoff].copy()
    
    # --- 8.1) Avaliação do Modelo TFT ---
    print("  - Avaliando o modelo Temporal Fusion Transformer...")
    
    try:
        with torch.inference_mode():
            predictions = tft.predict(val_loader, mode="raw")["prediction"]
        
        # Obter os valores reais diretamente do dataloader para garantir alinhamento perfeito
        actuals_flat = torch.cat([y[0] for x, y in iter(val_loader)]).flatten().cpu().numpy()

        # Isolar o quantil da mediana (índice 1 no terceiro eixo) e achatar
        tft_preds_median = predictions[:, :, 1].flatten().cpu().numpy()
        
        # Garantir que os tamanhos correspondem (caso o último batch seja menor)
        min_len = min(len(actuals_flat), len(tft_preds_median))
        actuals_flat = actuals_flat[:min_len]
        tft_preds_median = tft_preds_median[:min_len]

        mae_tft = mean_absolute_error(actuals_flat, tft_preds_median)
        rmse_tft = np.sqrt(mean_squared_error(actuals_flat, tft_preds_median))
        # Adicionar um pequeno epsilon para evitar divisão por zero no MAPE
        mape_tft = mean_absolute_percentage_error(actuals_flat + 1e-6, tft_preds_median)
    
    except Exception as e:
        print(f"  ❌ Erro ao avaliar o modelo TFT: {e}")
        traceback.print_exc()
        mae_tft, rmse_tft, mape_tft = -1, -1, -1


    # --- 8.2) Avaliação do Baseline: Regressão Linear ---
    print("  - Avaliando o modelo de Baseline (Regressão Linear)...")
    try:
        features = ['time_idx', 'Mes', 'PA_PROC_ID']
        categorical_features = ['Mes', 'PA_PROC_ID']
        target = 'Valor_Total'
        ...
        print(f"  ❌ Erro ao avaliar o modelo de Regressão Linear: {e}")
        traceback.print_exc()
        mae_lr, rmse_lr, mape_lr = -1, -1, -1

    # --- 8.3) Apresentar Resultados ---
    print("\n" + "="*75)
    print("Resultados da Avaliação (no período de Jan/2023 a Dez/2024)")
    print("="*75)
    print(f"{'Métrica':<10} | {'TFT (Seu Modelo)':<25} | {'Regressão Linear (Baseline)':<30}")
    print("-"*75)
    print(f"{'MAE':<10} | R$ {mae_tft:,.2f}{'':<14} | R$ {mae_lr:,.2f}")
    print(f"{'RMSE':<10} | R$ {rmse_tft:,.2f}{'':<14} | R$ {rmse_lr:,.2f}")
    print(f"{'MAPE':<10} | {mape_tft:.2%}{'':<21} | {mape_lr:.2%}")
    print("="*75)
    print("Interpretação:")
    print("- MAE: Em média, quanto o modelo erra (em Reais). Menor é melhor.")
    print("- RMSE: Similar ao MAE, mas penaliza mais os erros grandes. Menor é melhor.")
    print("- MAPE: Erro médio em porcentagem. Menor é melhor.")
    print("="*75 + "\n")

    # ==========================================================
    # NOVO: Seção 8.4 - Análise do Horizonte de Previsão (Nível Brasil)
    # ==========================================================
    print("\n" + "="*75)
    print("Análise de Performance por Horizonte de Previsão (Backtesting - Nível Brasil)")
    print("="*75)
    ...
    # Loop por cada PROCEDIMENTO para gerar a previsão recursiva de 24 meses
    grupos_backtest = df_backtest_hist.groupby("PA_PROC_ID")

    for i, (proc_id, df_g) in enumerate(grupos_backtest, start=1):
        print(f"  - Backtesting para Procedimento {proc_id}...")

        # Pular grupos com histórico muito curto
        if len(df_g) < max_encoder_length // 2:
    ...
    if backtest_predictions:
        ...
        df_val_comparison = pd.merge(
            df_val[["PA_PROC_ID", "Ano", "Mes", "Valor_Total"]],
            df_backtest_results[["PA_PROC_ID", "Ano", "Mes", "Valor_Previsto_Mediana"]],
            on=["PA_PROC_ID", "Ano", "Mes"]
        )
        ...
        print("\n--- Performance do TFT por Ano de Previsão (Nível Brasil) ---")
    ...
    # ----------------------------------------------------------
    print("\n🔮 Prevendo recursivamente 2025–2034 (120 meses) por grupo...")
    all_outputs = []
    grupos = df["PA_PROC_ID"].unique()
    for i, g in enumerate(grupos, start=1):
        print(f"[{i}/{len(grupos)}] → Prev {g}")
        df_g = df[df["PA_PROC_ID"] == g].copy()
        if df_g.empty: continue
        months_needed = 120
        produced = 0
        df_local = df_g.copy()
        df_local["time_idx"] = df_local.groupby("PA_PROC_ID").cumcount().astype("int32")
        last_real_time_idx = df_g["time_idx"].max()
        while produced < months_needed:
            ...
            try:
                ...
                print(f"  ❌ Erro durante tft.predict() para o grupo {g}: {e}"); traceback.print_exc(); break
            ...
            out_block = pd.DataFrame({
                "PA_PROC_ID": [g] * len(valor_mediana), "time_idx": list(future_time_idx)[:len(valor_mediana)],
            ...
            out_block_for_concat["Valor_Total"] = out_block_for_concat["Valor_Previsto_Mediana"]
            out_block_for_concat["Quantidade"] = 0.0
        ...
        final_cols = ["PA_PROC_ID", "time_idx", "Ano", "Mes", "Valor_Previsto_Mediana", "Valor_Melhor_Cenario", "Valor_Pior_Cenario"]
        all_outputs.append(df_future_result[final_cols])
        del df_local, df_future_result, df_g
        gc.collect()

    if all_outputs:
        final = pd.concat(all_outputs, ignore_index=True)
        final.to_csv(OUT_PATH, index=False)
        print(f"\n✅ Previsões finais com cenários e features do IBGE salvas em:", OUT_PATH)
    else:
