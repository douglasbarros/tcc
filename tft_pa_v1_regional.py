...
if __name__ == "__main__":
    ...
    OUT_PATH = BASE_PATH + "predicoes_TFT_v1_regional.csv"
    CHECKPOINT_PATH = BASE_PATH + "tft_v1_regional.ckpt"
    ...
    usecols = ["PA_PROC_ID", "Regiao", "Ano", "Mes", "Valor_Total", "Quantidade"]
    ...
    df_raw = df_raw.dropna(subset=["Valor_Total", "Ano", "Mes", "PA_PROC_ID", "Nome_Regiao"])

    print("🔄 Agregando dados por (Procedimento, Nome_Regiao, Ano, Mês)...")
    # MUDANÇA REGIONAL: Adicionar 'Nome_Regiao' ao groupby
    df = df_raw.groupby(["PA_PROC_ID", "Nome_Regiao", "Ano", "Mes"]).agg(
    ...
    df = df.sort_values(["PA_PROC_ID", "Nome_Regiao", "Ano", "Mes"]).reset_index(drop=True)
    # MUDANÇA REGIONAL: O time_idx agora é calculado para cada grupo (procedimento, Nome_Regiao)
    df["time_idx"] = df.groupby(["PA_PROC_ID", "Nome_Regiao"]).cumcount().astype("int32")

    print(f"Dados agregados para {len(df.groupby(['PA_PROC_ID', 'Nome_Regiao']))} grupos (séries temporais).")
    ...
    static_features = ["PA_PROC_ID", "Nome_Regiao"]
    ...
    training = TimeSeriesDataSet(
        ...
        group_ids=["PA_PROC_ID", "Nome_Regiao"],
    ...
    for regiao in regioes:
        ...
        preprocessor = ColumnTransformer(transformers=[('cat', OneHotEncoder(handle_unknown='ignore'), ['Mes', 'PA_PROC_ID'])], remainder='passthrough')
    ...
    coluna_geografica = "Nome_Regiao"
    grupos_backtest = df_backtest_hist.groupby(["PA_PROC_ID", coluna_geografica])
    ...
    for i, (group_ids, df_g) in enumerate(grupos_backtest, start=1):
        ...
        while produced < validation_months: # Prever por 24 meses
            ...
            out_block = pd.DataFrame({
                "PA_PROC_ID": proc_id, coluna_geografica: geo_id,
    ...
    if backtest_predictions:
        ...
        df_val_comparison = pd.merge(
            df_val[["PA_PROC_ID", coluna_geografica, "Ano", "Mes", "Valor_Total"]],
            df_backtest_results[["PA_PROC_ID", coluna_geografica, "Ano", "Mes", "Valor_Previsto_Mediana"]],
            on=["PA_PROC_ID", coluna_geografica, "Ano", "Mes"]
        )
    ...
    grupos = df.groupby(["PA_PROC_ID", "Nome_Regiao"])
    ...
    for i, (group_ids, df_g) in enumerate(grupos, start=1):
        ...
        df_local = df_local[(df_local["PA_PROC_ID"] == proc_id) & (df_local["Nome_Regiao"] == nome_regiao)].reset_index(drop=True)
        df_local["time_idx"] = df_local.groupby(["PA_PROC_ID", "Nome_Regiao"]).cumcount().astype("int32")
        ...
        while produced < months_needed:
            ...
            out_block = pd.DataFrame({
                "PA_PROC_ID": [proc_id] * len(valor_mediana),
        ...
        if previsao_sucedida:
            ...
            final_cols = ["PA_PROC_ID", "Nome_Regiao", "time_idx", "Ano", "Mes", "Valor_Previsto_Mediana", "Valor_Melhor_Cenario", "Valor_Pior_Cenario"]
