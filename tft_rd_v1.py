...
if __name__ == "__main__":
    ...
    OUT_PATH = BASE_PATH + "predicoes_TFT_RD_v1.csv"
    CHECKPOINT_PATH = BASE_PATH + "tft_RD_v1.ckpt"
    ...
    df_raw = pd.read_csv(CSV_PATH, low_memory=False, usecols=["PROC_REA", "Ano", "Mes", "Valor_Total"])
    ...
    df_raw = df_raw.dropna(subset=["Valor_Total", "Ano", "Mes", "PROC_REA"])
    df = df_raw.groupby(["PROC_REA", "Ano", "Mes"]).agg(
    ...
    df = df.sort_values(["PROC_REA", "Ano", "Mes"]).reset_index(drop=True)
    df["time_idx"] = df.groupby("PROC_REA").cumcount().astype("int32")
    ...
    training = TimeSeriesDataSet(
        ...
        time_idx="time_idx", target="Valor_Total", group_ids=["PROC_REA"],
        ...
        static_categoricals=["PROC_REA"], allow_missing_timesteps=True
    )
    ...
    try:
        ...
        categorical_features = ['Mes', 'PROC_REA']
        target = 'Valor_Total'
    ...
    grupos_backtest = df_backtest_hist.groupby("PROC_REA")
    ...
    if backtest_predictions:
        ...
        df_val_comparison = pd.merge(
            df_val[["PROC_REA", "Ano", "Mes", "Valor_Total"]],
            df_backtest_results[["PROC_REA", "Ano", "Mes", "Valor_Previsto_Mediana"]],
            on=["PROC_REA", "Ano", "Mes"]
        )
    ...
    all_outputs = []
    grupos = df["PROC_REA"].unique()
    for i, g in enumerate(grupos, start=1):
        print(f"[{i}/{len(grupos)}] → Prev {g}")
        df_g = df[df["PROC_REA"] == g].copy()
        print(f"    -> Histórico ({len(df_g)} meses).")

        if len(df_g) < max_encoder_length:
            print(f"    -> ⚠️ ATENÇÃO: Histórico insuficiente ({len(df_g)} meses) para o encoder_length de {max_encoder_length}. Pulando este grupo.")
        ...
        produced = 0
        df_local = df_g.copy()
        df_local["time_idx"] = df_local.groupby("PROC_REA").cumcount().astype("int32")
        ...
        while produced < months_needed:
            ...
            out_block = pd.DataFrame({
                "PROC_REA": [g] * len(valor_mediana), "time_idx": list(future_time_idx)[:len(valor_mediana)],
        ...
        final_cols = ["PROC_REA", "time_idx", "Ano", "Mes", "Valor_Previsto_Mediana", "Valor_Melhor_Cenario", "Valor_Pior_Cenario"]
